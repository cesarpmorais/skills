...
```diff
 src/
 ├── commands/
+│   └── run-skill.ts     # expands the slash command
 ├── sessions/
-└── transport.ts
+└── transport/
+    ├── client.ts
...
```
Bash(open path/to/teach-a-padawan-{description}.html)     # macOS
Bash(xdg-open path/to/teach-a-padawan-{description}.html)  # Linux
```
